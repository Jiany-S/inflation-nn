...
from .config import RESULTS_DIR, N_LAGS, SELECTED_FEATURES
...
def plot_preds_vs_actuals(model, X_test, y_test, y_test_sr, fname: str = "preds_vs_actuals.png"):
    ...
    assert len(idx) == len(preds), f"Index/preds length mismatch: {len(idx)} vs {len(preds)}"
...
def run_all_plots():
    ...
    if len(feature_names) != n_feat:
        print(f"[warn] feature_names({len(feature_names)}) != X_test width({n_feat}); aligning.")
